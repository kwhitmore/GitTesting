...
class XMLChangeLogSAXHandler extends DefaultHandler {
	...
	public void startElement(String uri, String localName, String qName,
		...
		try {
			if ("comment".equals(qName)) {
                ...
                if (atts.getValue("relativeToChangelogFile") == null) {
                       isRelativeToChangelogFile = true;
                }
                else {
                       isRelativeToChangelogFile = Boolean.
                               parseBoolean(atts.getValue("relativeToChangelogFile"));
                }
				handleIncludedChangeLog(fileName, isRelativeToChangelogFile,
                        databaseChangeLog.getPhysicalFilePath(), atts.getValue("revisionNo"), Boolean.
                               parseBoolean(atts.getValue("failOnError")));
				...
				for (URL fileUrl : resources) {
					...
					if (file.isDirectory()) {
						...
						for (File childFile : new TreeSet<File>(Arrays.asList(file.listFiles()))) {
                            ...
                            if (handleIncludedChangeLog(path, false, databaseChangeLog.getPhysicalFilePath(),"",failOnError)) {
								foundResource = true;
							}
						}
					} else {
	...
	protected boolean handleIncludedChangeLog(String fileName,
			boolean isRelativePath, String relativeBaseFileName, String revisionNo, Boolean failOnError)
            throws LiquibaseException, InterruptedException {
		if (!(fileName.endsWith(".xml") || fileName.endsWith(".sql") || fileName.endsWith(".fnc")
                  || fileName.endsWith(".bdy") || fileName.endsWith(".spc")  || fileName.endsWith(".vw")
                  || fileName.endsWith(".prc") || fileName.endsWith(".seq") || fileName.endsWith(".tbl")
                  || fileName.endsWith(".trg") || fileName.endsWith(".tpb") || fileName.endsWith(".mvw")
                  || fileName.endsWith(".tps") )) {
        ...
        if (revisionNo != null &&  revisionNo != "") {
            ...
            SVNRevision revision = SVNRevision.create(Integer.parseInt(revisionNo));
            File file = new File(fileName);

            rev.Update(file, revision, SVNDepth.EMPTY, true, false);
            rev.UpdateCompleted(file, revision);

            changeLog = ChangeLogParserFactory.getInstance().getParser(fileName, resourceAccessor).parse(fileName, changeLogParameters,
						resourceAccessor, revisionNo, failOnError);
        }
        else {
            changeLog = ChangeLogParserFactory.getInstance().getParser(fileName, resourceAccessor).parse(fileName, changeLogParameters,
